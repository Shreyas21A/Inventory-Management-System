...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) && preg_match('/^[0-9-]{10,15}$/', $phone)) {
        ...
        $stmt->bind_param("ssss", $sellerName, $email, $phone, $address);
        if ($stmt->execute()) {
            $success = "Seller added successfully!";
        } else {
            $error = "Error adding seller: " . htmlspecialchars($conn->error);
        }
        $stmt->close();
    } else {
...
<body class="bg-blue-50 font-sans">
    ...
    <div class="container mx-auto p-8">
        <h1 class="text-3xl font-bold text-indigo-800 mb-6 tracking-tight">Manage Sellers</h1>
        <div class="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto mb-8">
            ...
            <?php endif; ?>
            <h2 class="text-xl font-semibold text-indigo-700 mb-6">Add Seller</h2>
            <form method="post" action="" class="space-y-6">
                <div>
                    <label for="seller_name" class="block text-sm font-medium text-gray-700">Seller Name</label>
                    <input type="text" id="seller_name" name="seller_name" required class="mt-1 block w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 transition duration-200">
                </div>
                <div>
                ...
                <button type="submit" class="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-3 rounded-lg hover:bg-indigo-700 transform hover:scale-105 transition duration-200">Add Seller</button>
            </form>
        </div>

        <div class="bg-white p-8 rounded-xl shadow-lg">
            <h2 class="text-xl font-semibold text-indigo-700 mb-6">Seller List</h2>
            <?php if ($sellerResult->num_rows > 0): ?>
            ...
            <?php else: ?>
                <p class="text-gray-700">No sellers found.</p>
            <?php endif; ?>
        </div>
    </div>
